...
| `UPDATE_STRATEGY` | `delete_append` | Two-stream **continue-update** write op. Default = bounded **DELETE+APPEND** (fits a 4 GB VM); `=merge` selects the legacy **MERGE** (opt-out). ⚠ **Cost:** the default rewrites the **whole table once per 1024 update keys** — an 858 k-key window on a 115 M-row table = 839 rewrites (≈ 56 h / 2 TB egress on remote S3; ≈ 74 min on local disk). `=merge` is one pass and **fits an 8 GB VM for routine windows**: measured 2026-08-07 at **1.757 GB peak / 22 % of an 8 GiB cap, zero swap** (115.2 M-row table, 26 k-row window, `MERGE_MEMORY_MB=2048` + `MERGE_TARGET_PARTITIONS=1`). Memory tracks the **window**, not the table — the ~6.9 GB figure in two-stream-continue-update.md §2.3 is a *bootstrap* number. **Large windows (~858 k rows, i.e. after an outage) are still unmeasured**; for those prefer a one-shot `TABLE_RECONCILE_<table>=true`, which is single-pass and bounded regardless of host. See [two-stream-continue-update.md](two-stream-continue-update.md) §2–§3 and audit-findings §10.1. |
