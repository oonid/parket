...
| `UPDATE_STRATEGY` | `delete_append` | Two-stream **continue-update** write op. Default = bounded **DELETE+APPEND** (fits a 4 GB VM); `=merge` selects the legacy **MERGE** (opt-out). ⚠ **Cost:** the default rewrites the **whole table once per 1024 update keys** — an 858 k-key window on a 115 M-row table = 839 rewrites (≈ 56 h / 2 TB egress on remote S3; ≈ 74 min on local disk). `=merge` is one pass (17.8 min measured) but its memory scales with the **whole table** (~6.9 GB floor at 112 M rows, grows) and **cannot** be bounded — so it needs a roomy host, not an 8 GB VM. On a small host with a large update window, prefer a one-shot `TABLE_RECONCILE_<table>=true` (single pass, bounded). See [two-stream-continue-update.md](two-stream-continue-update.md) §2–§3 and audit-findings §10.1. |
