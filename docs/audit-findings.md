...
- **P1-r** — deferred follow-ups: (a) **connection-pooling half** — `extract()` opens a fresh `mysql::Conn` per call (`extractor.rs:56-59`); pooling must preserve the M2 breaker-truncation safety (the fresh-conn-per-call is a *deliberate* design, `extractor.rs:78-79`) so it's discard-on-truncation and Docker-only-provable — to be scoped separately. (b) once-per-table read/create paths still fresh-load (negligible: called once per table).
