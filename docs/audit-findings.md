...
- **O7** — `--check`/run parity (PARTIAL): a shared mode-resolver (`discovery::resolve_ts_col_and_mode`) now backs the **run + `--verify`** (see O12, `1b89161`), but **preflight still has its own `detect_mode` call** (`preflight.rs:109`) — adopting the shared resolver there is the remaining mode-parity work. Still open: no evolution check in preflight; S3 health-check written at bucket root ignoring `s3_prefix`; local mode probes nothing.
- **O8** — **done** (`ee09a7f`).
...
- **D1-r (Low-Med)** — `merge_batch` (`UPDATE_STRATEGY=merge` opt-out) still drops a new column's data: `schema_evolution_check` SELECTs the new column into the batch, but the MERGE op's clauses only touch known columns, so the extra column is dropped (the D1 bug, confined to the opt-out path). Documented on the fn, but not test-covered, and it's unverified whether delta-rs silently drops vs errors on a superset MERGE source. Fix: discriminating test + evolve the merge source or fall back to delete_append when a new column is present. Also **D1-r2 (Low)**: a column RENAME (drop+add) fails with the misleading "table was dropped" wording (safe — needs full refresh — but confusing). **D1-r3 (Info)**: always-Merge runs a proven-no-op schema reconciliation on every append; unbenchmarked on large multi-batch incremental loads.
...
- **O9** — failed-run state wipes last-success metadata + `schema_columns_hash` is write-only (computed + stored but never read — dead feature; either consume it for a schema-drift shortcut or drop it). `state.rs`.
- **O11** — `TABLE_TIMESTAMP` (`table_timestamp_col`) is validated in `resolve_ts_col_and_mode` REGARDLESS of the resolved mode, so a table with `TABLE_TIMESTAMP` set that resolves to full_refresh (which never uses the cursor) fails validation unnecessarily. Fix: validate the cursor only when the resolved mode consumes it (incremental/two-stream). Now that the resolver is shared (O12), this is a one-place change — but it's a **run-behavior change** (changes when the run bails), so it needs its own test, kept separate from O12's behavior-preserving extraction.
