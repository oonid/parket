...
- **O7** — `--check`/run parity (PARTIAL): the **mode-parity sub-item is DONE** (`a09642e`, O11) — run, `--verify`, and preflight all now resolve mode via the single shared `discovery::resolve_ts_col_and_mode`. Still open (non-mode parity gaps): no schema-evolution check in preflight; S3 health-check written at bucket root ignoring `s3_prefix`; local mode probes nothing.
- **O8** — **done** (`ee09a7f`).
