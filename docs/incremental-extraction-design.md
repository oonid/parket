# HWM (High Watermark) — Detection & Configuration Plan

> **Status:** planning document. Describes current behaviour and a proposed
> design for inspecting and predefining High Watermarks. Nothing here is
> implemented yet except where marked "today".
...
## 14.8 F10.4 — bound the two-stream MERGE memory (fit a 4 GB VM, accept slow)

> **Status: ✅ implemented + VALIDATED on real data (2026-06-11).**
> On deltalake 0.32.4 / datafusion 53.1.0. `merge_batch` builds a bounded `FairSpillPool`
> (`MERGE_MEMORY_MB`, default = `TARGET_MEMORY_MB`) + `DiskManagerBuilder` (`MERGE_SPILL_DIR`,
> default system temp), forces `prefer_hash_join=false`, and passes the session via the
> **mandatory** `MergeBuilder::with_session_state`. Wired through both writer adapters.
> Note: `with_disk_manager` is deprecated in df53 → used `with_disk_manager_builder` +
> `DiskManagerMode::Directories`.
>
> **Acceptance run (112M-row `orders`, `systemd-run -p MemoryMax=3500M -p MemorySwapMax=4G`,
> `TARGET_MEMORY_MB=512`, `MERGE_MEMORY_MB=2048`, spill on ext4):**
> **peak RSS 3107 MB (< 3500 cap), peak swap 5 MB (~0), 4.6 GB spilled to disk.** The merge
> that needed 21–25 GB unbounded now fits a 4 GB VM. ✅
>
> **But it first failed with `Too many open files` (errno 24):** the external sort spills into
> many files and the k-way merge exceeded the soft `NOFILE` limit (systemd scope default
> ~1024, not inherited from the shell's 1048576). **Fix shipped:** parket raises its own soft
> `RLIMIT_NOFILE` to the hard limit at startup (`main.rs::raise_nofile_limit`, libc
> get/setrlimit, best-effort + logged). So no `ulimit`/`LimitNOFILE` tuning is needed in
> normal deployments. Caveat: `MERGE_SPILL_DIR` must be **real disk, not tmpfs**.
>
> **Then a second failure (FD fixed): `Not enough memory to continue external sort`** — the
> 2048 MB pool exhausted during the merge-of-runs (merge needed +10 MB, 2.7 MB free). Run:
> peak RSS **4237 MB**, swap 4 MB, spill **14 GB**. Two lessons: (1) the merge external-sorts
> the **whole 112M-row target** (14 GB spill) to update ~100 rows — heavy regardless of pool;
> (2) RSS ≈ pool + **~2.2 GB overhead** (2 GB pool → 4.2 GB RSS), and the `--user --scope`
> `MemoryMax=3500M` was **not enforced** (RSS exceeded it without an OOM-kill — user scopes
> often lack memory delegation; use `sudo systemd-run --scope --uid=…` to enforce honestly).
> **Tuning shipped:** `MERGE_SORT_RESERVATION_MB` env (read in `merge_batch`) overrides
> datafusion's `sort_spill_reservation_bytes` (default 10 MB) — lowering it (1–2) can let a
> smaller pool finish. Primary lever remains a bigger `MERGE_MEMORY_MB`.
>
> **Reality check:** for THIS 112M-row table the full-target sort makes a real (enforced) 4 GB
> VM borderline (need pool big enough to complete the merge, but pool + 2.2 GB overhead must
> fit RAM). The durable small-VM fix is still §14.8.7 (stats-on-append + id-windowed merge,
> avoid sorting the whole target) — deferred per user preference to keep the full-data merge.
>
> Tests: CI clippy clean, 384 lib/bin + 8 integration (incl. `two_stream_inserts_and_merges_
> mutations_across_runs` end-to-end) + the bounded-pool unit test + the NOFILE-raise test.

### 14.8.1 The finding that motivates this
The F10.3 fixes worked: the second live run **succeeded** (`succeeded=8 failed=0`,
~29.5 min). Seeding reduced the update stream to **988 rows** (not millions), no crash.
But the in-run sampler caught:
```
PEAK RSS = 21992 MB | PEAK swap = 8599 MB
```
The spike was **not** the insert phase (~2 GB, sampled live). It was the **update
MERGE**: merging 988 rows took `extract_ms=230345` (the `completed_at` filesort over
112M unindexed rows) + `write_ms=334147` (5.5 min) and ~21.5 GB RSS + 8.4 GB swap.

Root cause: `merge_batch` builds its source via `SessionContext::new()` — datafusion's
**default `UnboundedMemoryPool`, no `DiskManager`**. delta-rs MERGE joins the source
against the **entire 112M-row target** (appended files carry no min/max `id` stats →
no file pruning), loading it all into the join → ~21 GB. Two consequences:
- **`TARGET_MEMORY_MB` has zero effect on the merge** — it bounds only the
  extract/append/full_refresh chunk, not the datafusion join. Lowering it to 256 will
  *not* shrink the merge.
- On a 4 GB VM the first run that has completions to merge will **OOM**, regardless of
  how the table was bootstrapped (full_refresh-first doesn't help — the merge is a
  per-run cost, not a bootstrap cost).

**Empirical confirmation (256 vs 1024, both fresh two-stream bootstraps of the same
112M-row table):**

| Metric | `TARGET_MEMORY_MB=1024` | `TARGET_MEMORY_MB=256` |
|---|---|---|
| Peak RSS | 21.5 GB (`21992 MB`) | **25.2 GB (`25760 MB`)** ⬆️ |
| Peak swap | 8.4 GB | 0.1 GB (machine-state noise) |
| Insert chunk | ~10M rows | ~2.1M rows (¼, as expected) |
| Update extract | 230 s | 231 s (the `completed_at` filesort — invariant) |
| Update merge | 334 s | 276 s (run-to-run variance) |
| Total wall-clock | 29m30s | 28m48s (~identical) |

Quartering the target made peak RSS **go up** (21.5 → 25.2 GB) — definitive proof the
merge is the peak and is uncontrolled by `TARGET_MEMORY_MB`. Wall-clock was unchanged
(merge filesort + join dominate, both target-invariant). The swap drop is *not* a 256
benefit — at 1024 the box was already swapping ~18 GB from other processes; here more
RAM was free when the merge hit. **Two-stream on this table needs ~21–25 GB at any
target.** F10.4 is therefore *required*, not an optimization.

**Decision (2026-06-11):** full_refresh-every-run is **not** an acceptable option for
this table — two-stream is the committed path. The target VM RAM is **not fixed at 4 GB**;
it remains an exploration variable. So F10.4's goal is a *tunable* merge bound
(`MERGE_MEMORY_MB`) — bound the merge, then explore the minimum-viable VM RAM vs
spill-speed trade-off, rather than hard-targeting 4 GB.

> Aside — bootstrap-via-full_refresh then switch to two-stream **does not resume today**:
> `overwrite_table` stamps `hwm_updated_at`/`hwm_last_id` but never `hwm_insert_id`, and
> `process_two_stream` has no insert-watermark seed → the insert stream re-appends all
> 112M rows (duplicates). That "resume" gap is a *separate* feature (seed `hwm_insert_id`
> from `MAX(id)`, or have full_refresh stamp both two-stream watermarks when cursors are
> configured) — not part of F10.4.

### 14.8.2 Goal
Make `merge_batch` complete within a fixed RAM budget by giving datafusion a **bounded,
spilling** memory pool instead of the unbounded default — trading speed (disk spill) for
survival. Target: the 112M-row update merge stays within a configured budget + overhead,
spilling to disk rather than OOM-killing on a 4 GB VM.

### 14.8.3 The lever
Replace `SessionContext::new()` in `merge_batch` with a context whose `RuntimeEnv` has:
1. **`FairSpillPool`** sized from config (the *spillable* pool; `GreedyMemoryPool` only
   errors on exceed — it does not spill).
2. **`DiskManager` enabled** (sorts/joins spill to a temp dir instead of erroring).
3. **`SessionConfig` with `datafusion.optimizer.prefer_hash_join = false`** — force a
   SortMergeJoin, because datafusion's **HashJoin historically does not spill** (a bounded
   pool would just make it error `ResourcesExhausted`). SortMergeJoin + external sort spill.

API sketch (verify exact names against the datafusion version deltalake 0.32 pins —
`RuntimeEnvBuilder` vs older `RuntimeConfig`; `DiskManagerConfig` vs `DiskManagerBuilder`):
```rust
let rt = RuntimeEnvBuilder::new()
    .with_memory_pool(Arc::new(FairSpillPool::new(merge_pool_bytes)))
    .with_disk_manager(DiskManagerConfig::NewSpecified(vec![spill_dir]))
    .build()?;
let cfg = SessionConfig::new()
    .with_target_partitions(target_partitions)        // bound parallelism → bound peak
    .set_bool("datafusion.optimizer.prefer_hash_join", false);
let ctx = SessionContext::new_with_config_rt(cfg, Arc::new(rt));
// build the deduped source DataFrame from THIS ctx (as today)
```

### 14.8.4 Spike RESULT (2026-06-11, against deltalake 0.32.4 / datafusion 53.1.0) — ✅ GO

Investigated `deltalake-core-0.32.4/src/operations/merge/mod.rs` + datafusion 53.1.0 source.

**Q1 — does delta-rs honor our session state? YES, but it must be passed explicitly.**
- `MergeBuilder` holds `state: Option<Arc<dyn Session>>` (line 172) and exposes a public
  **`with_session_state(state: Arc<dyn Session>)`** (line 422).
- `execute()` calls `resolve_session_state(this.state.as_deref(), …, || create_session().state())`
  (line ~1839): if we **don't** call `with_session_state`, `this.state == None` →
  it falls back to **`create_session().state()` = the default *unbounded* pool** (our 21 GB path).
  delta-rs does **NOT** auto-adopt the source DataFrame's context.
  → **`with_session_state` is MANDATORY.** (The "build the source from our ctx and it'll be
  used" assumption in §14.8.3 is *wrong* — keep building the source from our ctx for the dedup,
  but the merge execution only honors the explicitly-passed state.)
- The resolved state drives the **entire** physical plan: `state.create_physical_plan(merge_final)`
  (line 1563), source + target both `DataFrame::new(state.clone(), …)` (1004/1017). So the join
  runs under our pool.
- Caveat from the docstring: the passed state must wrap a **concrete `SessionState`** (use
  `Arc::new(ctx.state())`); otherwise the fallback policy kicks in. `SessionStateBuilder::new_from_existing`
  (line 867) preserves the RuntimeEnv (pool + disk manager).

**Q2 — will it spill vs error? YES (mechanism present).**
- Force a SortMergeJoin via `prefer_hash_join=false` (datafusion 53 HashJoin does not spill).
- `SortExec` (the sorts feeding the SMJ) **spills** to disk under a bounded pool + `DiskManager`
  (`datafusion-physical-plan-53.1.0/src/sorts/sort.rs` has spill). SortMergeJoin then streams;
  with a unique `id` key there are no duplicate runs to buffer → bounded memory.
- The big cost becomes the **external sort of the 112M-row target by `id`** (slow, spills to
  disk — the accepted trade). Still memory-bounded.

**datafusion 53.1.0 API names — all confirmed present:**
- `datafusion::execution::runtime_env::RuntimeEnvBuilder` → `.with_memory_pool(Arc<dyn MemoryPool>)`,
  `.with_disk_manager(DiskManagerConfig)`, `.build() -> Result<RuntimeEnv>`.
- `datafusion::execution::memory_pool::FairSpillPool::new(pool_size: usize)`.
- `datafusion::execution::disk_manager::DiskManagerConfig::{NewOs, NewSpecified(Vec<PathBuf>), Disabled}`.
- `SessionContext::new_with_config_rt(config: SessionConfig, runtime: Arc<RuntimeEnv>)`.
- `prefer_hash_join`: verify the exact setter in F10.4a — either `SessionConfig::with_prefer_hash_join(false)`
  or `cfg.options_mut().optimizer.prefer_hash_join = false`.

**Verdict: GO with the pool-lever (F10.4a).** The windowed fallback (§14.8.7) is **not** needed
unless empirical testing still OOMs. Corrected call shape:
```rust
let ctx = SessionContext::new_with_config_rt(cfg, Arc::new(rt));
let source = /* deduped DataFrame built from ctx, as today */;
let (table, _metrics) = DeltaTable::merge(source, predicate)
    .with_session_state(Arc::new(ctx.state()))   // ← MANDATORY, else default unbounded pool
    /* …when_matched/when_not_matched as today… */
    .await?;
```

### 14.8.5 Sizing (decouple from the extract budget)
- The merge runs *after* extract, so it can use most of RAM — don't shackle it to the
  256 MB extract batch.
- New env **`MERGE_MEMORY_MB`** (optional; default = `TARGET_MEMORY_MB`). On a 4 GB VM:
  `TARGET_MEMORY_MB=256`, `MERGE_MEMORY_MB=2500`.
- **The pool bounds only *tracked* allocations.** Actual RSS = pool + Arrow decode +
  untracked. Document "RSS exceeds `MERGE_MEMORY_MB`"; recommend ~50–60 % of spare RAM.

### 14.8.6 Disk-spill caveat
Spilling a large join writes **gigabytes** to the temp dir (≈ the target data scanned).
On a small VM with a small disk this can fill the disk. Make the spill dir configurable
(**`MERGE_SPILL_DIR`**, default system temp); document the free-disk requirement. Spilling
also makes the merge **much** slower — the accepted trade.

### 14.8.7 Fallback (if the spike fails) — F10.4b: stats-on-append + id-windowed merge
If delta-rs ignores our session, or the join won't spill:
1. Write **min/max `id` statistics** on the append path so target files are prunable.
2. Sort the update source by `id`, process it in **id-range windows**, MERGE each with
   predicate `id BETWEEN lo AND hi` → delta-rs prunes the target to the few overlapping
   files → bounded memory *and* far less I/O than spilling the whole join.

More code, but reduces the actual *work* (fewer files) rather than just spilling; scales
to any table size. Plan B only — reach for it if the pool lever can't bound delta-rs.

### 14.8.8 Config & docs
- New env: `MERGE_MEMORY_MB` (optional), `MERGE_SPILL_DIR` (optional). Validate positive
  int / path.
- `config.md`: a "Bounding MERGE memory" subsection (pool vs RSS, spill-disk requirement,
  the systemd-run validation).

### 14.8.9 Implementation steps (gated; snapshot before each haiku dispatch)
- **F10.4-spike** (throwaway): confirm delta-rs session-state injection + spill behavior;
  decide pool-lever vs windowed-fallback. *No commit.*
- **F10.4a** (`src/writer.rs`): build the bounded/spilling `SessionContext` in
  `merge_batch`; wire `MERGE_MEMORY_MB`/`MERGE_SPILL_DIR`. Anti-git guard; one file.
- **F10.4b** (`src/config.rs`): the two new env vars + validation + tests.
- **F10.4c** (docs): `config.md` + this §.
- (Only if the spike fails: **F10.4-fallback** = stats-on-append + id-windowed merge —
  re-plan as its own gated sequence.)

### 14.8.10 Tests
- Unit: assert the merge `SessionContext` is built with a bounded pool, disk manager, and
  `prefer_hash_join=false` (config introspection).
- Existing `merge_batch` upsert + dedup tests still pass (correctness unchanged).
- Spilling can't be unit-tested cheaply → **manual acceptance test**: run the `orders` update
  merge under `systemd-run --user --scope -p MemoryMax=3500M -p MemorySwapMax=0` and
  confirm it *completes* (spills) instead of OOM-killing.

### 14.8.11 Risks (ranked)
1. **delta-rs builds its own session internally** → pool ignored → windowed fallback.
   *(Spike resolves.)*
2. **Merge join won't spill even with `prefer_hash_join=false`** → bounded pool errors
   instead of fitting → windowed fallback. *(Spike resolves.)*
3. Spill fills the VM disk → configurable spill dir + docs.
4. Pool undersized vs untracked allocs → still OOM → tune `MERGE_MEMORY_MB` down.

### 14.8.12 Sizing findings from the acceptance runs (2026-06-11, real 112M-row `orders`)

All runs: fresh bootstrap, `TARGET_MEMORY_MB=512`, spill on ext4. The update merge processes
~100–9500 changed rows against the **full 112M-row target** every time. Top block = default
multi-partition (~14 sorters); bottom block = the shipped `MERGE_TARGET_PARTITIONS=1`.

| VM cap | `MERGE_MEMORY_MB` | Enforced? | Peak RSS | Peak swap | Spill | Result |
|---|---|---|---|---|---|---|
| (host, unbounded) | — (default pool) | n/a | 21–25 GB | — | — | ✅ completed (hash-build of the 112M target) |
| `--user` 3500M | 2048 | **no** (user scope, no mem delegation) | 4.2 GB | ~0 | 14 GB | ❌ `Too many open files` → **fixed** by NOFILE self-raise |
| `--user` 3500M | 2048 | no | 4.2 GB | ~0 | 14 GB | ❌ `Not enough memory to continue external sort` (pool exhausted) |
| `sudo` 3500M | 1536 + `RESERVATION_MB=2` | **yes** | 3282 MB (< cap) | 1883 MB | **57 GB** | ❌ pool exhausted at `ExternalSorterMerge[5]` (multi-level re-spill) |
| `sudo` 7500M | 4096 | **yes** | 5410 MB | 0 | 38 GB | ❌ pool exhausted at `ExternalSorterMerge[1]` — even a 4 GB pool starves |
| `sudo` 7500M | 4096 + **`PARTITIONS=1`** | **yes** | 7446 MB | **4088 MB** | ~0 | ✅ completed — but **streamed in-RAM** (no spill), over-buffered → 4 GB swap |
| `sudo` 7500M | 2048 + **`PARTITIONS=1`** | **yes** | **6908 MB** | **0** | ~0 | ✅ **completed clean** — no swap, no spill, merge 47 s (← recommended) |

**Conclusions:**
1. **`MERGE_MEMORY_MB` (the FairSpillPool) is a HARD internal cap, independent of VM RAM.**
   The 1536 MB run failed inside datafusion ("2.7 MB remain available for the total pool")
   while the OS still had headroom (RSS 3282 < 3500 cap, +1.8 GB swap unused). The pool must
   be large enough for the merge to **complete** — reservation tuning and RAM/swap headroom
   cannot substitute.
2. **RSS ≈ pool + ~2.2 GB overhead** (target scan + Arrow + runtime). Measured: 2 GB pool →
   4.2 GB RSS.
3. **A too-small pool degenerates into pathological re-spilling** — 1536 MB pool spilled
   **57 GB** (4× amplification) via multi-level merge and still couldn't finish.
4. **`sudo systemd-run --scope --uid=…` enforces `MemoryMax`; `--user --scope` did not**
   (user manager lacks memory-controller delegation here).
5. **The real wall is PARALLELISM, not pool size** (discovered when the 4096 MB pool ALSO
   failed). The `ExternalSorterMerge[N]` bracket is the **partition index** — datafusion ran
   the external sort with `target_partitions` = CPU count (~14) parallel sorters, **all sharing
   the one FairSpillPool**. Each sorter's merge phase competes for the pool, so even 4096 MB
   fragments and the merge starves. **We never set `target_partitions`** → it defaulted to all
   cores. *(This supersedes the earlier premature "4 GB infeasible / ~8 GB floor" conclusion.)*
6. **Fix (implemented + VALIDATED 2026-06-11): `target_partitions = 1`** (default; override via
   `MERGE_TARGET_PARTITIONS`). Both `PARTITIONS=1` runs **completed** — fan-out was indeed a wall.
   But this revealed the next layer (7–8).
7. **With one partition the merge STREAMS — it does not do a full external sort and barely
   spills** (`spill ~0`, merge 47–69 s, RSS ~7 GB ≪ the 14 GB target). So `MERGE_MEMORY_MB` is an
   **in-memory buffering cap, not a disk-spill trigger**: `4096` over-buffered → 7.4 GB RSS **+
   4 GB swap**; `2048` → 6.9 GB RSS, **no swap, no spill**. **Smaller pool = tighter RSS** (until
   the floor), and it never meaningfully spills. *(Corrects conclusion 2's "RSS ≈ pool + 2.2 GB"
   — that was the multi-partition model.)*
8. **There is a ~6–7 GB working-set FLOOR for 112M rows, ~independent of the pool** (RSS moved
   only 6.9→7.4 GB across a 2× pool change). So **4 GB is genuinely infeasible** for this table's
   full-data merge — reinstating the earlier conclusion, now for the *right* reason (intrinsic
   streaming working set, not fan-out). **Recommended config: 8 GB VM, `TARGET_MEMORY_MB=512` /
   `MERGE_MEMORY_MB=2048` / `MERGE_TARGET_PARTITIONS=1` → ~6.9 GB RSS, clean.** Size the VM to
   **peak RSS**, not to the pool.
9. **Runtime is DB-bound, not parket.** The slow runs (up to 2 h) were `user+sys ≈ 0 CPU` —
   parket I/O-waiting on MariaDB. Insert-stream extract degraded **6 s → 73 s → 90 s per chunk**
   across runs while the merge stayed ~47–69 s. Likely MariaDB↔parket resource contention on a
   shared host (the merge's ~7 GB / earlier spill I/O evicting the DB buffer pool). Tune the DB
   / co-location, not parket, for speed.
10. The intrinsic cost is still **processing the whole 112M target every merge**. §14.8.7
    (stats-on-append + id-windowed merge) remains the only escape **below the ~7 GB floor** /
    onto a 4 GB VM — deferred per user preference to keep full-data semantics.
